---
name: aria-blog
description: Aria blog authoring & publishing pipeline for the BaroSit community. Use when asked to write/publish an Aria blog post, pick the next blog topic, research blog topic candidates, or seed a blog post to production. Modes - publish (default, one post end-to-end with human review gates) | research (expand the verified topic backlog). Korean/English/Japanese trilingual, seed-migration based.
---

# Aria 블로그 발행 Skill

BaroSit 커뮤니티에 Aria(자세 코치) 명의로 3언어(ko/en/ja) 블로그 글을 발행하는 전 과정.

## 원칙: 문서 역할 분담 (중복 금지)

- **`docs/blog-topics.html` = 콘텐츠 상태의 단일 진실 문서** — 발행 현황(§1)·백로그(§3)·오귀속 함정 목록(§3 상단)·글 작성 체크리스트(§5). **이 skill에 복사하지 않는다.** 여기엔 절차만.
- **이 SKILL.md = 절차의 단일 진실 문서** — 단계·검토 게이트·마이그레이션 스펙·검증 방법.
- 메모리(`barosit-blog-content`)는 세션 간 포인터일 뿐. 원칙이 충돌하면 blog-topics.html과 이 파일이 우선.

## 모드 파싱

인자에서: `research`가 있으면 리서치 모드, 아니면 **발행 모드(기본)**. 주제가 인자로 주어지면 1단계 주제 선정을 그 주제 검증으로 대체.

---

## 발행 모드 (기본)

**대원칙: 사람 검토는 노이즈가 아니라 품질 파이프라인의 핵심 단계다.** 지금까지 매 글 발행 직전 사용자 검토가 실제 오류(§5 인과 미끄러짐, 제목 과잉 약속, 자기모순)를 잡았다. 게이트를 건너뛰고 발행하지 말 것.

### 0. 로드

1. `docs/blog-topics.html` 전체를 읽는다 (§1 발행 현황, §3 백로그+오귀속 함정, §5 체크리스트).
2. 최근 발행 글 2~3편의 시드 마이그레이션(`supabase/migrations/*seed_aria_blog*.sql`)을 훑어 형식·톤 감각을 로드한다.

### 1. 주제 선정 → [게이트 A: 사용자 확인]

- §3 백로그에서 후보 선정. **배치 규칙 3개 필수**: ①유형 교차(직전이 debunk면 권고형, 반대도) ②테마 교차(같은 테마 연속 금지) ③기발행 글과 중복 위험 노트 확인(예: "스트레칭vs근력"은 하루2분글과 중복 위험 실화).
- **⭐배치 규칙만으로 고르면 실패한다(2026-07-30 실증: 한 회차에 주제가 두 번 기각됨).** 배치 규칙을 통과한 후보에 §5 체크리스트의 **ⓐⓑ 두 관문**을 추가로 적용할 것:
  - **ⓐ 이미 곤란을 겪는 독자가 존재하는가.** 후보마다 두 칸을 채운다 — ①곤란을 겪는 독자는 누구인가 ②읽고 나면 무엇이 달라지는가. 안 채워지면 **근거 검증에 들어가기 전에 버린다**. 기각된 "무거운 건 허리 펴고 들어라"는 근거가 튼튼했는데도 결론이 "근거가 없다" 하나뿐이라 **ko 초안을 다 쓴 뒤 게이트 C에서 "하나마나한 내용"으로 기각**됐다. debunk가 특히 잘 빠지는 함정.
  - **ⓐ의 보강 — "독자가 존재하는가"가 아니라 "그 독자가 이 블로그의 독자인가"**(2026-08-04, 허리 복대가 ko 초안까지 쓰인 뒤 "공감하기 어렵다"로 기각). 근거가 튼튼하고 곤란을 겪는 독자가 실재해도, 그 사람들이 **이사·물류·헬스장에** 있고 우리 독자가 **책상 앞에** 있으면 실패한다. 판정: **"이 물건·행동을 하는 사람의 전형적인 장소가 어디인가"**를 한 문장으로 써 볼 것.
  - **⭐ⓓ 그 연구가 비교한 두 집단이 지금도 존재하는가**(2026-08-04, 손목터널이 게이트 B를 끝낸 뒤 기각). 낡은 앵커에 **"더 최신 연구가 있나"만 묻고 "그때의 대조가 오늘도 성립하나"는 묻지 않은 것**이 사각지대였다. Shiri 2015의 대조군은 "컴퓨터를 거의 쓰지 않는 사무직"인데 2026년엔 없다(노출 변수 이름이 `computer/typewriter use`였다). 반대로 8/03 요통 글은 2010년 앵커여도 "누워 있기 vs 평소 활동" 대비가 그대로라 살았다. **논문의 나이가 아니라 대조군의 나이를 볼 것.** ✅구제책=현행 가이드라인 개관이 같은 방향이면 노후를 밝히면서 논지가 선다. 그것조차 없으면 버린다.
  - **⭐ⓔ 그 근거가 시험한 '행동'이 독자가 실제로 하는 행동인가**(2026-08-04, 워킹패드가 ko 초안까지 쓰인 뒤 "홈트용으로 쓰지 업무 보면서 쓰는 예가 많나"로 기각). 근거는 전부 **업무 *중* 걷기**(그래서 타이핑 속도를 쟀다)인데 국내 제품은 **홈트 기구**였다 → 소유자가 있어도 근거가 그 사람이 하는 일을 설명하지 못한다. ⚠**물건의 위치를 독자의 존재로 착각하지 말 것**("책상 밑에 두는 물건이니 우리 독자"는 착시였다). ⚠**연구 표본이 유독 작으면 "왜 작은가"를 물을 것** — 워킹패드 참가자 70명은 경계가 아니라 신호였다.
  - **ⓑ 그 debunk가 신뢰 자산을 깎지 않는가.** 대상이 장비·통념·상품이면 안전하고, **의료 행위·검사·전문가 판단**에 닿으면 위험하다. 기각된 "MRI 디스크 소견"은 게이트 B를 통과하고도 **"MRI에 대한 불신을 조장하는 것 아닌가"**로 기각됐다.
- 후보·이유(유형/테마/중복 + ⓐⓑ 판정)를 제시하고 **사용자 확정을 받은 뒤** 진행.
- **기각돼도 검증 결과는 §3에 남긴다** — 원문 대조는 그대로 자산이고, "되살리려면 어떤 축으로 바꿔야 하는지"까지 적어두면 다음 회차가 거의 공짜가 된다.

### 2. 근거 검증 → [게이트 B: 검증 리포트 제시]

- **원문 fetch 없이는 한 줄도 쓰지 않는다. 기억·검색엔진 요약 인용 금지.** 검색엔진은 서로 다른 논문을 합쳐 허위 서지를 만든다(반복 확인된 사고 패턴) — §3 상단 '오귀속 함정' 목록을 먼저 대조.
- 근거마다: ①1저자·연도·저널을 원문에서 직접 대조 ②핵심 수치(효과크기·CI·n) 원문 확인 ③**"그 연구가 다루지 않은 것"** 명시(치료↔예방, 측정 안 한 지표, 기간 한계) ④"효과 없음(연구됨)" vs "근거 없음(미연구)" 구분 ⑤COI(자금원·저자 소속) ⑥동명이인 주의(Andersen LL≠CH 전례).
- **⭐소비재 주제(베개·침구·보조기구 등)는 COI 를 '있나 없나'가 아니라 '몇 편에 있나'로 볼 것**(2026-08-06 실측 — 검토 6편 중 3편에 업계 이해관계). 🚨**가장 위험한 형태 = 그 제품을 파는 사람이 자기 방법의 효과를 보고한 연구**: 베개 높이 조정의 임상 효과를 보인 유일한 연구가 **1저자가 베개 연구소 주주 겸 대표이사**였고 **대조군 없는 전후 비교**였다 → 단독 앵커 금지를 넘어 **인용 불가**로 판정했다(유일한 근거라고 해서 기준을 낮추지 말 것). ✅**반대 방향 COI 는 강한 신뢰 재료다** — 같은 주제에서 저자 2명이 베개 기업 소속인 리뷰가 "최적 높이는 아직 특정할 수 없다"고 결론냈고, **그 사실을 본문에 노출하니 논지가 오히려 단단해졌다**(§5 ⓑ 안전판의 첫 실적용).
- **백로그 앵커가 오래됐으면(5년+) 후속·최신 메타를 반드시 찾아본다.** 2026-07-27 발행에서 백로그 앵커(2006 메타) 위에 **642개 검정 규모의 2024 메타**가 존재함을 발견 — 최신 것이 있는데 옛 수치만 쓰면 근거가 낡아 보인다.
- **⭐"유의하지 않았다"를 인용할 때는 출판편향 보정치의 방향까지 본다**(2026-08-05 적발). Albulescu 2022 의 성과 결과는 주 분석에서 비유의(d=.16, CI −.04~.37)였는데 **Egger 검정이 유의**(p=.048)했고 **trim-and-fill 보정치가 d=0.22[0.02,0.43]으로 오히려 유의**해졌다. 즉 **보정이 결론을 약화시키는 게 아니라 강화하는 방향**이라, 그 논문으로 "효과가 없다"를 단정하면 부정직해진다. 출판편향은 대개 효과를 부풀리지만 **항상 그렇지는 않다** → 비유의 결과를 쓸 때 보정치를 확인하지 않으면 "근거 없음(미검출)"을 "효과 없음"으로 바꿔치기하게 된다. 문형은 **"이 자료에서는 또렷하게 잡히지 않았다"**까지.
- **⚠같은 논문 안에서 p 값이 두 개일 수 있다**(같은 건에서 적발) — 결과 서술문은 `p = .17`, Table 과 초록은 `p = .116`(효과크기·CI 는 동일). **표·초록을 정본으로 삼고 본문에는 p 대신 신뢰구간을 쓸 것.** 수치를 옮길 때 서술문과 결과표를 교차 확인한다.
- **⭐인용할 수치마다 "이건 무엇에 의존하는 값인가"를 먼저 묻는다**(2026-07-30, 유저 질문 "퍼센트의 의미가 뭐지?"로 적발). 사후확률·오즈비·상대위험도는 **기준선이 바뀌면 의미가 통째로 달라진다** — 특히 **사후확률은 사전확률(그 진료 환경의 유병률) 의존**이라 독자 상황에 대입되지 않는다. 임상의용 수치·도구(Wells 점수 등)를 독자용으로 옮기면 자가진단 도구가 된다. **한 구절로 설명할 수 없는 수치는 본문에서 뺀다.** 뺀 자리에 "왜 옮기지 않았는지"를 한 문단으로 쓰면 오히려 글이 단단해진다(실증됨).
- **정오표(erratum)·republication 표시를 확인한다.** PubMed 초록 상단의 `Erratum in` / `Republished in` 줄을 볼 것. 2026-07-30 앵커(Downie 2013)에 정오표가 붙어 있어 PMC로 원문을 받아 대조했고 결과는 "쉼표 하나 + 부록 인용번호"였지만, **확인하지 않고 쓰는 것과 확인하고 쓰는 건 다르다**.
- **백로그에 적힌 서지·수치도 검증 대상이다**(그 자체를 신뢰하지 말 것). 실제로 "보강 X 2013 d=0.59"가 **동일 저자·동일 연도의 다른 논문**(메타 ↔ RCT) 수치와 뒤섞여 기재돼 있었다. 앵커의 **연구 설계(메타/RCT/코호트)를 원문에서 직접 확인**할 것.
- **⭐백로그의 괄호 요약은 원문 결론의 요약이 아니라 '누군가의 기억'일 수 있다**(2026-08-06, 백로그 자체가 틀린 네 번째 사례). 경추 베개 앵커가 <s>"라텍스 통증 소폭↓, 정렬·수면 무효"</s>로 적혀 있었으나 원문은 ①**rubber(고무)**이고 ②통증은 "소폭"이 아니라 **p<0.001 로 유의**(**효과 크기가 작은 것과 통계적으로 유의하지 않은 것은 다르다**) ③핵심 결과 하나(목 장애 −0.506)가 **통째로 누락** ④"정렬 무효"가 실제로는 **"재질은 무효, 모양·높이는 유효"**였다. ④가 결정적인데, **그 글의 유일한 행동 축이 백로그에서는 '무효'로 지워져 있었다** — 백로그만 믿었으면 독자가 가져갈 게 없는 글이 나온다. → 백로그 요약의 **긍정/부정 방향까지** 원문에서 짚을 것.
- **⭐백로그 수치는 "점추정치인지 신뢰구간 끝인지"까지 확인한다**(2026-08-03 실증, 하마터면 정반대 글). 백로그가 Dahm 2010 을 "통증은 차이 없음(**0.02**), 고편향 제외 시 0.22"로 적고 있었는데 리뷰에 그런 서술이 없었고, **0.02 는 CI 하한**이었다(실제 = SMD 0.22, 95% CI 0.02–0.41). 그대로 썼으면 "활동 유지는 통증엔 효과 없다"는 글이 나왔다. **한 앵커에 숫자가 둘 이상 적혀 있으면 어느 쪽이 점추정치인지 원문에서 짚을 것.**
- **⭐"리뷰 총계 ≠ 그 분석의 표본"은 코호트만의 함정이 아니다** — 같은 건에서 백로그가 리뷰 전체(10 RCT·1,923명)를 급성 요통 결과에 붙여 놨으나 그 통합은 **2건 401명**이었다. Hagger-Johnson(응답 12,778 vs 분석 10,937)과 같은 패턴이 **SR/메타에서 반복**된다. 초록의 "We included N trials (n=…)"를 결과 문장의 표본으로 옮겨 쓰지 말 것.
- **정오표 확인의 형제 규칙 — 철회(WITHDRAWN)·병합 여부를 본다.** Cochrane 리뷰는 서로 병합되면서 **옛 리뷰가 철회되고 근거 등급·CI 가 재계산**된다. 실제로 Hagen KB CD001254 는 CD007612 로 병합·철회됐는데, **철회본은 같은 수치를 high quality 로, 현행본은 moderate 로** 등급했고 기능 CI 도 달랐다(0.05–0.45 → 0.09–0.49). 검색 요약은 철회본 값을 반환할 수 있다 → **현행본 값을 쓸 것.**
- **⭐찾아낸 '최신'도 그대로 믿지 말 것 — 최신 메타가 인용 불가일 수 있다**(2026-08-04). 앵커 5년+ 규칙을 돌려 찾은 Akbar 2025 *Narra J* 메타는 **보고된 SMD 1.33 이 자기 95% CI(−2.09~−0.57) 밖**에 있었고 **I²=97%**였다. 부호 오류로 보이지만 논문이 확정해 주지 않으므로 인용 금지. **최신 것을 찾는 규칙과 그것을 검증하는 규칙은 별개다.**
- **⭐앵커가 낡았는데 후속 RCT 가 없을 때의 구제책 = 가이드라인 개관**(2026-08-03 실증). 2010년 리뷰가 최신인 주제였는데, *Lancet Rheumatol* 의 **가이드라인 개관 계보(2001·2010·2018·2026)**를 찾아 "현행 지침이 여전히 같은 방향"임을 보일 수 있었다. 덕분에 **근거의 노후를 고지에 정직하게 밝히면서도 논지가 섰다.** 이 계보는 요통 주제에서 재사용 가치가 크다(Oliveira/Koes/Maher).
- **원문 접근 경로(막히는 순서대로 시도)** — 2026-07-28 발행에서 전부 겪음:
  1. PubMed **웹페이지가 reCAPTCHA를 반환**할 수 있다(연속 요청 시). CAPTCHA는 풀지 말고 **NCBI E-utilities**로 우회: `curl -s "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?db=pubmed&id=<PMID1>,<PMID2>&rettype=abstract&retmode=text"` — 여러 PMID를 한 번에 받을 수 있고 공식 초록이라 신뢰 가능.
  1-a. **PMC 도 막힐 수 있다** — `efetch db=pmc` 가 *"The publisher does not allow downloading of the full text in XML form"* 을 반환하는 저널이 있고(J Chiropr Med 등), 그때 **PMC 웹페이지로 우회하려 하면 reCAPTCHA** 가 뜬다(2026-08-06). **CAPTCHA 는 풀지 않는다** → 그 논문은 미사용으로 확정하고 시드 주석에 남긴다. ⚠특히 **초록에 표본 수가 없는 논문**은 이 경로가 막히면 인용 자체가 불가능해진다.
  1-b. **SAGE(journals.sagepub.com)는 봇 검증 페이지를 띄운다** — curl 403, 브라우저로도 "보안 확인 수행 중"에서 멈춘다. **봇 검증은 우회하지 않는다.** unpaywall(`api.unpaywall.org/v2/<doi>`)로 대체 OA 경로를 확인하고, 없으면 **그 논문은 미사용으로 확정**하고 그 사실을 시드 주석에 남긴다(2026-08-05, Albulescu 2025 일기연구). ⚠초록만 확보된 논문을 "방향만 인용"하는 것도 금지 — 출처 목록에도 넣지 않는다.
  2. **초록으로 부족하면(층화 수치·저자 한계 서술) 리포지터리의 저자 원고**를 찾는다 — White Rose(eprints.whiterose.ac.uk)·PMC·대학 리포지터리. 페이월 논문도 accepted manuscript 는 공개인 경우가 많고, **한계 서술 전문**이 여기 있다.
  3. PDF 텍스트 추출: 이 환경엔 **poppler(pdftotext) 없음 → Read 도구의 PDF 렌더도 실패**한다. `python3` + **pypdf** 는 있으니 `PdfReader(...).pages[i].extract_text()` 로 텍스트를 뽑아 grep 할 것.
- **⭐페이월 본문의 수치는 "저자 자신의 다른 문헌"에서 찾는다**(2026-07-31 실증, 재사용 가치 높음). Lally 2010의 유명한 "평균 66일"이 **출판사 초록에 없고**(초록엔 범위 18~254일만) 본문은 페이월이라 막혔는데, **1저자가 공저한 리뷰**(Gardner, **Lally**, Wardle · *Br J Gen Pract* 2012 — 저널 사이트 전문 공개)에서 "plateau after an average of 66 days"를 찾아 해결했다. 검색 요약과 결정적으로 다른 점은 **저자 본인이 쓴 동료심사 문헌**이라는 것. 찾는 법: PubMed에서 `<1저자>[au]` + 주제어로 **그 저자의 후속 리뷰·해설**을 훑을 것. 이때 출처는 **그 문헌으로 밝힌다**(원 논문 본문을 본 척하지 말 것).
- **정오표 확인 규칙은 실제로 큰 오인용을 막았다**(2026-07-31). Buyalskaya 2023 *PNAS*의 헬스장 습관 형성 시간이 정오표에서 **122~226일(4~7개월) → 68~78일(2~3개월)**로 바뀌어 있었다. 인터넷에 도는 건 철회된 원래 수치다. **초록 상단 `Erratum in` 줄을 보고, 있으면 정오표 본문까지 읽을 것**(PMC의 정오표는 `db=pmc`로 전문이 받아진다).
- **출판사 사이트가 403이면 PMC로 우회**(2026-07-30 실증): BMJ·NICE 모두 curl·WebFetch에 403을 준다. PMC에 있으면 `efetch.fcgi?db=pmc&id=<PMCID>&rettype=xml`로 **전문 XML**을 받을 수 있고(정오표 본문도 여기서 확인), **NICE는 브라우저 도구(preview_start/get_page_text)로 읽힌다** — 권고 번호·문구를 그대로 인용해야 하니 원문 확인 필수. PDF는 `pypdf`(poppler·Read의 PDF 렌더는 이 환경에서 안 됨).
- **Cochrane 전문은 이 환경에서 안 받아진다** — PMC 에 PMCID 가 있어도 `efetch db=pmc` 가 *"The publisher of this article does not allow downloading of the full text in XML form."* 를 반환한다. 다만 **cochranelibrary.com 은 브라우저 도구로 초록·plain language summary·버전 이력(철회 여부)까지 읽힌다.** 대부분의 경우 초록에 효과크기·CI·등급이 다 있으므로 충분하다.
- **Lancet 계열은 curl 403 이지만 브라우저로는 CC BY 전문이 읽힌다** — `preview_start` 로 연 뒤 `get_page_text`(앞부분) + `javascript_tool` 의 `document.querySelector('article').innerText` **슬라이싱**으로 뒷부분을 꺼낸다(get_page_text 는 3만 자에서 잘리고 offset 이 없다).
- **⭐PubMed 저자명 색인 함정 — 성이 뒤집혀 색인되면 검색으로 존재 자체가 안 잡힌다**(2026-08-06 적발). 베개 메타분석의 1저자는 **Johnson Pang Chun-Yiu**인데 PubMed 는 **"Chun-Yiu JP"**로 색인해 두어 `Pang[au]` 로는 안 나온다. 홍콩·싱가포르계 3연음 이름에서 반복될 수 있다. → **제목 문구로 직접 검색**하고 **저자명은 Semantic Scholar 로 교차 확인**할 것. 백로그가 성으로 적어 둔 앵커가 PubMed 에서 안 잡히면, 없는 논문이 아니라 이 경우일 수 있다.
- **PubMed 미색인 저널 함정**: PubMed에서 안 잡힌다고 "없는 논문"으로 판단하지 말 것(예: *Health Psychology Review*). 출판사 페이지가 403이면 **Semantic Scholar API**(`api.semanticscholar.org/graph/v1/paper/DOI:<doi>?fields=…`)로 서지를, **healthevidence.org·McMaster Optimal Aging Portal** 같은 큐레이션 DB로 초록·수치를 대조한다. 검색 요약 자체를 출처로 삼는 것과는 다르다.
- 컨텍스트가 큰 검증은 Explore/general-purpose 에이전트에 위임하고 **검증 리포트만 받는다**. 단 에이전트 결과도 서지·수치는 원문 링크 재확인.
- 리포트(근거별 결론·경계·함정 회피 확인)를 사용자에게 제시. 근거가 흔들리면 여기서 중단하고 주제 재선정. **이번 검증에서 새로 적발한 오귀속·COI는 그 자리에서 §3 상단 목록에 추가한다**(발행 후로 미루지 말 것).

### 3. ko 집필 → [게이트 C: 사용자 검토 필수]

- 집필 전 **§5 체크리스트를 다시 열고** 항목별 대조하며 쓴다 (3중 경계·상황별 분기 착지·BaroSit 연결 원칙·서비스명 `BaroSit(바로씻)` 첫 언급 병기·어휘 뉘앙스·red flag 고지 등 — 전부 §5가 진실 소스).
- 구조: 번호 섹션(§1~§5) → (자연스러울 때만) BaroSit 연결+CTA → /science 링크 → 출처 목록(수치·CI 포함) → 의학 고지.
- **⭐관련 글을 언급하면 반드시 실제 URL을 붙인다**(2026-08-03 신설). "앞선 글에서 다룬 적이 있습니다"로 끝내지 말고 다음 줄에 `→ barosit.com/community/p/<같은 언어판 UUID>`. 그동안 시드 주석엔 "크로스링크"라고 적어 놓고 본문엔 언급만 해서 **발행 57편의 내부 링크가 0건**이었다(실측). SSR은 `linkifyBarositUrls()`가 자동으로 `<a>`로 승격하므로 평문으로 적으면 된다. ⚠**해시태그는 쓰지 않는다** — 웹페이지 본문의 `#키워드`는 구글 랭킹 요인이 아니고 글의 톤만 깎는다.
- 제목·부제 점검: **우리 근거로 답할 수 없는 걸 약속하지 않는가.** 마지막 섹션은 독자 상황별 분기(하는 중/곤욕/안 함)로 행동을 쥐여주고 질문 전환으로 착지.
- **⭐행동을 제시했으면 "그래서 얼마나/몇 개"까지 답했는지 스스로 물을 것**(2026-08-06 신설 — 유저 지적 *"높이가 중요하다고 했잖아. 그럼 얼마의 높이가 좋은건지 밝혀야 될 것 같아"*). 초안이 "종류가 아니라 높이를 보세요"에서 멈춰 **또 하나의 하나마나(ⓐ 함정)가 될 뻔했다.** debunk 가 "A가 아니라 B다"로 착지하면 독자의 다음 질문은 **반드시 "그럼 B는 얼마?"**이고, 거기서 멈추면 축만 옮겼을 뿐 상태는 안 바뀐다. ✅**찾아본 결과 수치가 없어도 손해가 아니다** — 이 건에서는 "몇 cm"에 근거가 없다는 것과 **왜 없는지**(체격·침구 의존이라 애초에 단일 숫자로 답할 수 없는 종류의 질문)가 오히려 더 쓸모 있는 내용이었다. 처리 = 실측 앵커가 하나라도 있으면 **한계와 함께** 주고, 없으면 **없다는 사실과 그 이유**를 쓰고, 행동은 "숫자를 맞히기"가 아니라 **"자기 몸으로 확인할 수 있는 조건"**으로 옮긴다(7/30 사후확률을 뺀 자리에 이유를 쓴 것과 같은 처리).
- **⭐집필 중 효과 크기를 만나면 반드시 체감 단위로 옮긴다**(2026-08-05 신설, §5 ⓕ). 경계를 다 지키고도 **"의미있는 결과가 약해보여. 어떤 영향이 있다는 건지 느낌이 안 오네"**로 반려된 데서 나왔다. `d=0.35` 를 적고 "관례상 작은 값"이라고만 덧붙이면 **정직함이 그대로 무의미로 읽힌다**. 세 가지로 처리:
  ①**공통언어 효과 크기로 환산** — "두 집단에서 한 명씩 무작위로 뽑아 비교하면 A 쪽이 나은 경우가 **10번 중 6번**". **차이가 없을 때의 값(5번)을 기준선으로 반드시 함께** 제시해야 크기가 읽힌다. 같은 값의 다른 표현이라 부풀림이 아니지만 **환산 사실을 출처 목록에 밝히고** 정규성 가정을 본문에 한 구절로("종 모양으로 퍼져 있다고 보고"). 계산 = `Φ(d/√2)`. 참고값 d=.16→5.5 / .35→6 / .5→6.4 / .8→7.1.
  ②**크기 판정 옆에 '비용'을 놓는다** — "작다"는 언제나 무엇과 비교하느냐의 문제. "관례상 작은 값입니다. 다만 여기서 지불하는 비용은 5분입니다"로 **판단을 독자에게 넘기면** 과장 없이 크기가 되살아난다.
  ③**크기가 작으면 일관성을 앞세운다** — 8/05 글의 진짜 심장은 0.35 가 아니라 **여덟 나라·네 직업군·네 종류 휴식활동인데 I²=0%** 라는 대비였다. 초안은 한 문장으로 흘렸고, 섹션 하나를 통째로 주자 근거가 살아났다.
  ⚠**구조도 함께 볼 것**: 불확실한 이야기가 **세 섹션 연속**이면 개별 문장이 다 정확해도 전체 인상은 "모르겠다"가 된다. **확실한 것 → 왜 작지 않은가 → 불확실한 것** 순으로 배치하면 같은 내용이 '결론'이 아니라 '기대의 조정'으로 읽힌다.
- 초안을 사용자에게 제시하고 **검토·수정을 받은 뒤** 다음 단계로.
- **게이트 C에서 "이 부분 굳이 필요한가" 축약 요청이 오면**: 경계를 삭제하지 말고 §5의 '경계는 전진 문장으로' 원칙대로 <u>수치 단서는 한 구절로 압축</u> + <u>미검증 영역은 다리 문장으로 전환</u>해서 다시 제시한다. 그래도 빼길 원하면 **그 수치도 함께 뺀다**(수치는 남기고 단서만 빼는 조합만 금지). 축약 요청은 대체로 옳다 — 실제로 2026-07-27 발행에서 본문이 여덟 문장 짧아지면서 더 좋아졌다.

### 4. en/ja 작성

- ko 확정본 기준으로 작성하되 **직역 금지 — 각 언어 원어민 문장으로 따로 쓴다.** 제목은 언어별 실검색어 반영(마이그레이션 주석에 검색어 기록). en/ja 서비스명은 `BaroSit` 그대로.

### 5. 시드 마이그레이션 작성

파일 2개, **git 커밋하지 않는다**(유저 지시 2026-07-07 — prod에만 apply, untracked로 남김):

- `supabase/migrations/<YYYYMMDD>010000_seed_aria_blog_<slug>.sql` (ko)
- `supabase/migrations/<YYYYMMDD>020000_seed_blog_<slug>_en_ja.sql` (en+ja)

형식 (기존 시드 파일이 살아있는 예시):

```sql
-- 상단 주석: 주제·유형(debunk/권고형)·직전 글과의 균형·근거 검증 요약(수치·경계)·정직 경계 목록
SET session_replication_role = replica;  -- anon은 is_agent=true 삽입 불가 → BEFORE 트리거 우회
INSERT INTO public.posts
  (id, title, content, author_name, category, is_agent, agent_role, user_id, password_hash, language, translation_group_id)
VALUES ('<고정 UUID>', '<제목>', $body$...$body$,
  'Aria', '📝 블로그', true, 'coach', NULL, '', '<ko|en|ja>', '<ko 글 UUID>')
ON CONFLICT (id) DO NOTHING;
SET session_replication_role = DEFAULT;
```

- UUID는 `uuidgen | tr A-Z a-z`로 언어별 3개 생성·고정. **ko UUID = 3개 언어 공통 `translation_group_id`** (한 그룹·한 댓글 스레드).
- `password_hash`는 빈 문자열 `''` 필수(NOT NULL 드리프트), `author_name` 필수.
- 본문 내 작은따옴표는 `$body$` 달러쿼팅이라 이스케이프 불필요(제목은 `''`).

### 6. prod 적용

- **Supabase MCP `apply_migration`으로 적용.** CLI `supabase db push` 금지(remote-only 이력 042244/042336 때문에 이력 수술 유발).
- **적용 전 반드시 `get_project`로 대상이 BaroSit(`kllcnllkcewnutxodwhx`)인지 확인** — MCP가 조직 스코프라 QueTick 등 다른 프로젝트 오조작 위험.
- ko → en/ja 순서로 2개 적용.

### 7. 검증 (3건 모두)

- SSR: `curl -s https://barosit.com/community/p/<ko UUID>`에 og:title·JSON-LD 반영 확인(en/ja UUID도).
- 사이트맵: `curl -s https://barosit.com/community-sitemap.xml`에 3개 URL 포함 확인. (Cloudflare PoP 전파 지연 있음 — 미반영이면 몇 분 뒤 재시도.)
- ko 본문에 `BaroSit(바로씻)` 병기 렌더 확인.

### 8. 사후 처리

1. `docs/blog-topics.html` 갱신: §1 발행 표에 행 추가(UUID·근거·정직 처리 요약), §3 백로그 해당 항목 취소선, §5에 이번에 새로 확립된 원칙이 있으면 추가. **이 docs 변경은 커밋한다**(시드 SQL은 제외: `git add docs/blog-topics.html`).
2. 메모리 `barosit-blog-content` 갱신(발행 요점·UUID·다음 주제 방향).
3. **이 SKILL.md 갱신**: 이번 발행에서 절차상 배운 것(새 함정·순서 변경)이 있으면 반영 — 이 skill은 살아있는 문서다.
4. 사용자에게 안내: **네이버 "웹페이지 수집" 수동 요청**(clean URL, 하루 한도 — 사람만 가능). 구글은 사이트맵 자동.

---

## 리서치 모드

백로그 확충. 산출물은 글이 아니라 **blog-topics.html §3 갱신**.

1. §3 현황 파악: 유형별 잔량 확인. **백로그는 debunk(특히 장비)로 기울기 쉬움 → 권고형 후보를 의도적으로 확보**(톤 균형 재발 방지).
2. 클러스터 3~4개(예: 행동 습관/장비/작업 환경/운동)로 나눠 **병렬 에이전트 리서치**. 각 에이전트에 요구: 후보별 앵커 논문 **원문 fetch 검증**(1저자·연도·저널·수치·COI), 검색 의도/실검색어, 기발행 글과의 중복 판단.
3. 에이전트 결과의 서지를 §3 오귀속 함정 목록과 대조 + 의심 건은 직접 재확인.
4. 후보를 유형(권고형/debunk 행동/debunk 장비)별로 정리해 사용자와 우선순위 논의 → §3에 반영(근거 링크·중복 노트·함정 노트 포함). 새로 발견한 오귀속은 §3 상단 목록에 추가.
