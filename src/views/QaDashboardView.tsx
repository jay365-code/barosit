...
const RAW_TEST_CASES: RawTestCase[] = [
  ...
  //   'PCI 자체 카드폼 제거'(commit 2173c55) 정책으로 삭제됨 — 카드 입력은 Toss 호스티드 UI에 위임.
  {
    id: "BILL-04",
    category: "Billing",
    title: "Toss Payments 빌링 키 발급 연동 모의 시뮬레이션",
  ...
  },
  {
    id: "BILL-05",
    category: "Billing",
    title: "카드 마스킹 정보 및 카드사 바인딩 (PCI-DSS 준수)",
  ...
  },
  {
    id: "BILL-06",
    category: "Billing",
    title: "구독 요금 플랜 임의 조작 변조 탐지 및 방어 (RLS & Trigger)",
  ...
  },
  {
    ...
    expected: "useEntitlement 의 서버 재검증으로 수 초~검증주기 내에 FREE 로 자동 강등되고, admin_notifications 에 plan_cache_mismatch 경보가 적재되어야 합니다(결제 이력 있으면 info, 없으면 warning). (§7 E3-②)",
    status: "Untested",
    actualResult: "",
    platforms: ["Web", "Mac", "Windows"]
  },
  ...
  },
  {
    ...
    expected: "src/auth/useEntitlement.ts 가 user_subscriptions 재조회 + reportPlanMismatch(plan_cache_mismatch) 경보를 포함하고, MonitorView·useMonitoringEngine 가 useEntitlement() 의 plan 을 사용해야 합니다(localStorage 직접 신뢰 금지).",
    status: "Untested",
    actualResult: "",
    platforms: ["Web", "Mac", "Windows"]
  },
  ...
  // 3. 자세 모니터링 엔진 및 기능 격하 (Monitoring & Degradation)
  {
    id: "MONI-01",
    category: "Monitoring",
    title: "웹캠 활성화 및 Pose Landmark 오버레이",
  ...
  },
  {
    id: "MONI-05",
    category: "Monitoring",
    title: "FREE 플랜 동기화 전송 차단 (비용 최적화)",
  ...
  },
  {
    id: "MONI-06",
    category: "Monitoring",
    title: "PRO 플랜 실시간 5분 주기 동기화 자동 타이머",
  ...
  },
  {
    id: "SYNC-02",
    category: "Offline",
    title: "네트워크 단절 중 자세 이벤트 로컬 무손실 적재",
  ...
  },
  {
    id: "SYNC-03",
    category: "Offline",
    title: "인터넷 재연결 감지 시 자동 벌크 동기화 복구",
  ...
  // 5. 커뮤니티 Q&A 기능 (Q&A) — 웹 마케팅 사이트의 #/community 라우트(Marketing.tsx) 전용. 데스크톱 앱 미노출 → Web only.
  {
    id: "COMM-01",
    category: "Q&A",
    title: "질문 게시물 신규 작성 및 실시간 목록 피드 인서트",
  ...
  },
  {
    id: "COMM-02",
    category: "Q&A",
    title: "상세 진입 시 조회수 갱신 및 좋아요 카운터",
  ...
  },
  {
    id: "COMM-03",
    category: "Q&A",
    title: "게시글 수정/삭제 제한 검증 (사용자 본인 판별 RLS)",
  ...
  // 6. 실시간 어드민 관제 센터 (Admin)
  {
    id: "ADMN-01",
    category: "Admin",
    title: "어드민 권한 체크 기반 관제 센터 진입 통제",
  ...
  },
  {
    id: "ADMN-02",
    category: "Admin",
    title: "실시간 종합 서비스 통계 및 다이내믹 SVG 차트 렌더링",
  ...
  },
  {
    id: "ADMN-03",
    category: "Admin",
    title: "가입자 요금 플랜 어드민 마스터 직접 강제 변조 조작",
  ...
  },
  {
    id: "ADMN-04",
    category: "Admin",
    title: "어드민 마스터 Q&A 직통 답변 및 부적절 콘텐츠 영구 삭제 데스크",
    method: "어드민 화면 - [커뮤니티 관리] 탭에서 특정 글을 선택하고 어드민 답변 등록 폼으로 댓글을 달거나 부적절 글을 직접 삭제해 봅니다.",
  ...
  },
  {
    id: "ADMN-05",
    category: "Admin",
    title: "실시간 텔레메트리 알림 자동 인서트 구독 (Supabase Realtime)",
  ...
  },
  {
    id: "ADMN-06",
    category: "Admin",
    title: "알림 수신 시 중요도(Severity)별 Web Audio 신디사이저 실시간 효과음 합성",
  ...
  },
  {
    id: "ADMN-07",
    category: "Admin",
    title: "알림 수신 토스트 팝업 슬택 및 자동 디스미스",
  ...
  },
  {
    id: "ADMN-08",
    category: "Admin",
    title: "90일 초과 만료 정보 수동 데이터 퍼징 및 콘솔 로그 가시화",
  ...
  },
  {
    id: "ADMN-09",
    category: "Admin",
    title: "품질 검증 QA용 가상 덤프 데이터 일괄 주입기",
...
const VERIFY_META: Record<string, { tier: VerifyTier; note: string }> = {
  ...
  "BILL-C03": { tier: "code", note: "grep useEntitlement.ts 'user_subscriptions' 및 'reportPlanMismatch'; MonitorView·useMonitoringEngine 에 useEntitlement() 호출." },
...
export function QaDashboardView() {
  const [testCases, setTestCases] = useState<TestCase[]>(() => {
    ...
    if (saved) {
      try {
        ...
        // 혹시 기존 구조와 병합
        return INITIAL_TEST_CASES.map(item => {
          const match = parsed.find((p: any) => p.id === item.id);
          return match ? { ...item, status: match.status, actualResult: match.actualResult, method: match.method || item.method } : item;
        });
      } catch (e) {
        console.warn("Failed to load local QA storage, using default values", e);
      }
    }
  ...
  // 실제 수행 결과 텍스트 변경
  const handleResultChange = (id: string, text: string) => {
    setTestCases(prev =>
      prev.map(item => (item.id === id ? { ...item, actualResult: text } : item))
    );
  };

  // 시험 방법 커스텀 수정
  const handleMethodChange = (id: string, newMethod: string) => {
    setTestCases(prev =>
      prev.map(item => (item.id === id ? { ...item, method: newMethod } : item))
    );
  };

  // 전체 리셋
  const handleReset = () => {
    if (!confirm("모든 테스트 진행 사항과 기록한 실제 결과들이 완벽하게 리셋됩니다. 계속하시겠습니까?")) return;
    setTestCases(INITIAL_TEST_CASES);
    localStorage.removeItem("barosit:qa_progress");
    alert("테스트 데이터가 완전하게 초기화되었습니다.");
  };
  ...
  const handleExportJson = () => {
    ...
    const filename = `barosit_qa_results_${dateStr}.json`;
  ...
  const handleImportJson = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        if (!Array.isArray(imported)) throw new Error("배열 포맷이 아닙니다.");

        const merged = testCases.map(item => {
          const match = imported.find((imp: any) => imp.id === item.id);
          if (match) {
            return {
              ...item,
              status: ["Untested", "Pass", "Fail", "N/A"].includes(match.status) ? match.status : "Untested",
              actualResult: typeof match.actualResult === "string" ? match.actualResult : "",
              method: typeof match.method === "string" ? match.method : item.method
            };
          }
          return item;
        });
        ...
        setTestCases(merged);
        alert(`성공적으로 ${imported.length}건의 테스트 체크리스트 기록을 성공적으로 가져와 병합 완료했습니다!`);
      } catch (err: any) {
        alert("QA JSON 불러오기 실패: 파일 데이터 구조가 올바르지 않습니다. " + err.message);
      }
    };
    reader.readAsText(file);
    // 동일 파일 재선택 가능하게 인풋값 비움
    e.target.value = "";
  };
  ...
  return (
    <div
      style={{
        minHeight: "100vh",
        background: "radial-gradient(circle at 50% 50%, #151a1e 0%, #0e1113 100%)",
        color: "#f3f6f9",
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", display: "flex", flexDirection: "column", gap: 32 }}>
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 16, flexShrink: 0 }}>
            <div
              style={{
                ...
                height: 48,
                borderRadius: 14,
                background: "linear-gradient(135deg, #5b8c7a, #355348)",
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
                fontSize: 22,
                boxShadow: "0 0 20px rgba(91, 140, 122, 0.4)",
              }}
            >
              🩺
            </div>
          ...
          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            ...
            <button
              ...
              style={{
                background: "linear-gradient(135deg, #5b8c7a, #43695b)",
                color: "#fff",
                border: "none",
                borderRadius: 12,
                padding: "11px 20px",
              ...
              onMouseEnter={e => {
                ...
                e.currentTarget.style.boxShadow = "0 6px 20px rgba(91, 140, 122, 0.4)";
              }}
              onMouseLeave={e => {
                e.currentTarget.style.transform = "none";
                e.currentTarget.style.boxShadow = "0 4px 15px rgba(91, 140, 122, 0.25)";
              }}
            >
              📤 결과 파일 저장 (Export JSON)
            </button>
            ...
            <button
              ...
              onMouseEnter={e => {
                ...
                e.currentTarget.style.borderColor = "rgba(201, 92, 92, 0.4)";
              }}
              onMouseLeave={e => {
                e.currentTarget.style.background = "rgba(201, 92, 92, 0.1)";
                e.currentTarget.style.borderColor = "rgba(201, 92, 92, 0.2)";
              }}
            >
              🔄 초기화
            </button>
        ...
        >
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span style={{ fontSize: 13, opacity: 0.6, fontWeight: 600 }}>테스트 진행률 (Progress)</span>
              <span style={{ fontSize: 18, fontWeight: 800, color: "#5b8c7a" }}>{progressPercent}%</span>
            </div>
            <div style={{ height: 10, background: "rgba(255,255,255,0.05)", borderRadius: 5, overflow: "hidden" }}>
              <div
                style={{
                  ...
                  background: "linear-gradient(90deg, #3c5e52, #5b8c7a)",
                  borderRadius: 5,
            ...
            <div style={{ fontSize: 11, opacity: 0.4 }}>
              전체 {totalCount}개 중 {totalCount - untestedCount}개 검증 완료 (성공: {passCount}, 실패: {failCount}, N/A: {naCount})
            </div>
          </div>
          ...
          {[
            ...
            >
              ...
              <div style={{ fontSize: 11, opacity: 0.4, marginTop: 2 }}>{m.sub}</div>
              {m.icon && (
                <div
                  style={{
                    position: "absolute",
        ...
        >
            ...
            {["All", "Auth", "Billing", "Monitoring", "Settings", "Offline", "Q&A", "Web", "Admin", "Lifecycle", "Desktop"].map(cat => {
              ...
              return (
                <button
                  ...
                  style={{
                    background: isActive ? "rgba(91, 140, 122, 0.2)" : "transparent",
                    color: isActive ? "#5b8c7a" : "#8a9aa8",
                    border: isActive ? "1px solid rgba(91, 140, 122, 0.4)" : "1px solid transparent",
                    borderRadius: 10,
                    padding: "8px 16px",
                    fontSize: 13,
                  ...
                  onMouseLeave={e => {
                    if (!isActive) e.currentTarget.style.color = "#8a9aa8";
                  }}
                >
                  {cat === "All" ? "전체" : cat} ({count})
                </button>
              );
            })}
          </div>
          ...
          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{ fontSize: 12, opacity: 0.5 }}>상태:</span>
              <select
            ...
            <input
              type="text"
              placeholder="ID, 검증제목, 시험방법 등으로 검색..."
              value={searchQuery}
              ...
              onFocus={e => (e.currentTarget.style.borderColor = "#5b8c7a")}
        ...
        <section style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {filteredCases.length === 0 ? (
            ...
            >
              🔍 조건에 만족하는 테스트 시나리오가 대시보드에 존재하지 않습니다.
            </div>
          ) : (
            filteredCases.map(item => {
              ...
              const statusStyles = {
                Untested: { bg: "rgba(138, 154, 168, 0.15)", color: "#8a9aa8", border: "rgba(138, 154, 168, 0.3)", label: "미수행" },
                Pass: { bg: "rgba(91, 140, 122, 0.15)", color: "#5b8c7a", border: "rgba(91, 140, 122, 0.4)", label: "성공 (Pass)" },
                Fail: { bg: "rgba(201, 92, 92, 0.15)", color: "#c95c5c", border: "rgba(201, 92, 92, 0.4)", label: "실패 (Fail)" },
                "N/A": { bg: "rgba(92, 143, 201, 0.15)", color: "#5c8fc9", border: "rgba(92, 143, 201, 0.4)", label: "N/A" }
              }[item.status];
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    border: `1px solid ${isExpanded ? statusStyles.color : "rgba(255, 255, 255, 0.05)"}`,
                    borderLeft: `4px solid ${statusStyles.color}`,
                    borderRadius: 18,
                    overflow: "hidden",
                    transition: "all 0.3s cubic-bezier(0.25, 0.8, 0.25, 1)",
                    boxShadow: isExpanded ? "0 10px 30px rgba(0,0,0,0.25)" : "none",
                  }}
                >
                  {/* 헤더 행 (요약 정보) */}
                  <div
                    ...
                    onMouseEnter={e => {
                      if (!isExpanded) e.currentTarget.style.background = "rgba(255, 255, 255, 0.01)";
                    }}
                    onMouseLeave={e => {
                  ...
                  >
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 16 }} onClick={e => e.stopPropagation()}>
                      {/* 빠른 상태 셀렉터 버튼 스택 */}
                      <div style={{ display: "flex", background: "#111", borderRadius: 10, padding: 3, border: "1px solid rgba(255,255,255,0.05)" }}>
                        {(["Untested", "Pass", "Fail", "N/A"] as const).map(st => {
                          const isSel = item.status === st;
                          const stColors = {
                            Untested: { bg: "rgba(138, 154, 168, 0.15)", color: "#8a9aa8" },
                            Pass: { bg: "rgba(91, 140, 122, 0.2)", color: "#5b8c7a" },
                            Fail: { bg: "rgba(201, 92, 92, 0.2)", color: "#c95c5c" },
                            "N/A": { bg: "rgba(92, 143, 201, 0.2)", color: "#5c8fc9" }
                          }[st];
                          ...
                          return (
                            <button
                              ...
                              style={{
                                background: isSel ? stColors.bg : "transparent",
                                color: isSel ? stColors.color : "#555",
                                border: "none",
                                borderRadius: 8,
                                padding: "6px 12px",
                                fontSize: 11,
                              ...
                              onMouseEnter={e => {
                                if (!isSel) e.currentTarget.style.color = "#bbb";
                              }}
                              onMouseLeave={e => {
                                if (!isSel) e.currentTarget.style.color = "#555";
                              }}
                            >
                              {st === "Untested" ? "미수행" : st}
                            </button>
                    ...
                    >
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginTop: 16 }}>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                          <label style={{ fontSize: 12, fontWeight: 700, color: "#5b8c7a", display: "flex", alignItems: "center", gap: 6 }}>
                            <span>🧪</span> 시험 방법 (Test Method) - <span style={{ opacity: 0.5, fontWeight: 500 }}>직접 편집 가능</span>
                          </label>
                          <textarea
                            value={item.method}
                            ...
                            onFocus={e => (e.currentTarget.style.borderColor = "rgba(91, 140, 122, 0.5)")}
                        ...
                        {/* 2. 기대 결과 */}
                        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                          <span style={{ fontSize: 12, fontWeight: 700, color: "#d9a752", display: "flex", alignItems: "center", gap: 6 }}>
                            <span>🎯</span> 기대 결과 (Expected Outcome)
                          </span>
                          <div
                            style={{
                              background: "rgba(217, 167, 82, 0.03)",
                              border: "1px solid rgba(217, 167, 82, 0.15)",
                              borderRadius: 12,
                              padding: 12,
                              color: "#e3e9f0",
                              fontSize: 12,
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                        <label style={{ fontSize: 12, fontWeight: 700, color: "#fff", display: "flex", alignItems: "center", gap: 6 }}>
                          <span>✍</span> 실제 수행 결과 및 이슈 기록 (Actual Results & Remarks)
                        </label>
                        <textarea
                          placeholder="여기에 실제 QA 테스트 결과를 자유롭게 기록하세요. (예: 정상 패스됨 / 특정 카테고리 클릭 시 반응 지연 1초 있음 등)"
