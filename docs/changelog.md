...
- **유저별 좋아요(하이브리드)** [migrations 080000·090000]: 로그인 유저는 DB 조인 테이블(`post_likes`/`comment_likes`) + `toggle_post_like`/`toggle_comment_like` SECURITY DEFINER RPC 로 **다기기 동기화·1인1추천·정확한 토글**. 게스트는 기존 localStorage 유지(하이브리드). `posts.likes`/`comments.likes` 카운트 컬럼은 그대로 증감해 **기존 수치 보존**. 좋아요 **취소**(decrement RPC). 프론트는 `isPostLiked`/`isCommentLiked` 헬퍼로 분기, `user` 늦은 복원 대비 liked 집합 재로드 effect. **로컬 DB+브라우저 E2E 검증**(추천→취소→새로고침 상태 유지, DB 행 생성/삭제 확인).
- **댓글/답글 UX**: 카드 통계 **👍·💬·👁 아이콘+숫자**(따봉 클릭=추천, 팝 애니메이션) · 큰 업보트 박스 제거 · **인라인 답글 입력**(해당 댓글 아래) · **연결선+들여쓰기 스레드** · **"답글 N개" 접기/펼치기**(기본 접힘) · 답글엔 답글버튼 숨김(1단계 일치) · 텍스트 라벨 제거(아이콘화).
- **작성자 활동 모달**: 회원 이름 클릭 → 그 회원 글·댓글 모아보기(원글 조인) → 클릭 시 스레드 이동 + **해당 댓글 스크롤·하이라이트**(답글이면 부모 스레드 자동 펼침).
- **폴리시**: 상대시간(방금 전·N분 전) · alert→토스트 · 모달 ESC/포커스 · 댓글 수 카드 동기화 · 댓글 정렬(작성순/인기순) · 본인 댓글 수정 · 영어 복수형(i18next `_one`/`_other`) · 글 목록 "더 보기" 페이지네이션.
- **실시간 댓글** [migration 100000]: `comments` realtime publication + `REPLICA IDENTITY FULL`, 활성 글 구독 effect. ⚠️ 구독 연결(SUBSCRIBED)은 확인했으나 **로컬 realtime 스택에서 라이브 이벤트 전달 미검증**(`realtime.subscription` 0행 — 로컬 컨테이너 등록 이슈, 코드는 표준 패턴). 원격에서 재확인 필요. 미동작 시 앱은 무해하게 degrade(작성·이동 시 refetch).
- **i18n** ko/en/ja 전반(추천 토글·상대시간·정렬·수정·더보기·작성자모달 등). **배포**: migrations 080000~100000 원격 `db push` + main 푸시→Cloudflare Pages.
- 검증: tsc 통과, 로케일 JSON 유효, 콘솔 에러 없음. 빌드(`build:web`) 성공.
